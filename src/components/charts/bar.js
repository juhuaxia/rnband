        ...
        const bars = this.state.nodata ? null : (() => {
            ...
            for(let i = 0; i < this.state.barData.length; i++) {
                ...
                for(let j = 0; j < item.length; j++) {
                    arr.push((
                        <Path key={i + j / 10} d={item[j].path} fill="none" stroke={this.state.legend[j]['color']} strokeWidth={this.state.barWidth} strokeLinecap="round" onPress={() => this._pathClickHandler(i, j)}/>
                    ));
                }
            }
            return arr;
