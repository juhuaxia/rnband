...
import {
    StyleSheet, View, Dimensions, Text, PanResponder, PixelRatio, Platform
} from 'react-native';
import Svg, { 
...
class ChartSleep extends IComponent {
    ...
    */
    ...
    _getXScaleIndexByLocationX(lx = 0) {
        const np = (lx - marginLeft - this.state.yScaleLeftWidth) / (this.state.width - marginLeft - marginRight - this.state.yScaleLeftWidth - this.state.yScaleRightWidth);
        for(let i = 0; i < this.state.data.length; i++) {
            if(np >= this.state.data[i].pos.x1 && np < this.state.data[i].pos.x2) {
                return i;
            }
        }
        return -1;
        ...
        if(idx === -1) {
            this.setState({ 
                indicator: {
                    ...
                    data: this.props.indicator.data
                } 
            });
        } else {
            const data = this.state.data[idx];
